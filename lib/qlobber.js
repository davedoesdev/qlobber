...
- `{Integer} cache_splits` How many `topic.split` results to cache. When you pass in a topic, it has to be split on the `separator`. Caching the results will make using the same topics multiple times faster at the cost of extra memory usage. Defaults to `0` (no caching). The number of split results cached is limited by the value to pass here.
    ...
    if (this._split_cache) {
        ...
        if (r === undefined) {
            ...
            if (this._split_cache.size > this._cache_splits) {
                for (let t in this._split_cache.keys()) {
